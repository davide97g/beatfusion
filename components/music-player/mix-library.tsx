...
import { Play, Edit, Copy, Download, Upload, Trash2, FileText, Calendar } from "lucide-react"
...
export function MixLibrary({ onLoadMix }: MixLibraryProps) {
  ...
  const { savedMixes, isLoading, deleteMix, duplicateMix, exportMix, importMix, exportAllMixes, clearAllMixes } =
    useMixStorage()
  ...
  const filteredMixes = savedMixes.filter((mix) => mix.name.toLowerCase().includes(searchTerm.toLowerCase()))
  ...
  const downloadExport = () => {
    const filename = selectedMix ? `${selectedMix.name}.json` : "all-mixes.json"
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        <div className="flex-1">
          <Input placeholder="Search mixes..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
        </div>
        ...
        <AlertDialog>
          <AlertDialogTrigger asChild>
            <Button variant="outline" size="sm" disabled={savedMixes.length === 0}>
          ...
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                This will permanently delete all saved mixes. This action cannot be undone.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
              <AlertDialogCancel>Cancel</AlertDialogCancel>
              <AlertDialogAction onClick={clearAllMixes}>Clear All</AlertDialogAction>
            </AlertDialogFooter>
          </AlertDialogContent>
        </AlertDialog>
      </div>
      ...
      {filteredMixes.length === 0 ? (
        <div className="text-center py-12 text-muted-foreground">
          ...
          <p className="text-lg mb-2">{searchTerm ? "No mixes found" : "No saved mixes"}</p>
          <p className="text-sm">
            {searchTerm ? "Try a different search term" : "Create and save your first mix to see it here"}
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredMixes.map((mix) => (
            <Card key={mix.id} className="p-4 hover:shadow-md transition-shadow">
              <div className="space-y-3">
                ...
                <div className="flex flex-wrap gap-1">
                  {mix.sections.slice(0, 3).map((section, index) => (
                    <Badge key={index} variant="outline" className="text-xs text-foreground bg-background">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Button size="sm" variant="outline" onClick={() => onLoadMix(mix)}>
                    <Edit className="w-3 h-3 mr-1" />
                    Edit
                  </Button>
                  <Button size="sm" variant="outline" onClick={() => duplicateMix(mix.id)}>
                    <Copy className="w-3 h-3" />
                  </Button>
                  <Button size="sm" variant="outline" onClick={() => handleExportMix(mix.id)}>
                  ...
                  <AlertDialog>
                    ...
                    <AlertDialogContent>
                      <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                          Are you sure you want to delete "{mix.name}"? This action cannot be undone.
                        </AlertDialogDescription>
                      </AlertDialogHeader>
                      <AlertDialogFooter>
                        <AlertDialogCancel>Cancel</AlertDialogCancel>
                        <AlertDialogAction onClick={() => deleteMix(mix.id)}>Delete</AlertDialogAction>
                      </AlertDialogFooter>
                    </AlertDialogContent>
                  </AlertDialog>
                </div>
      ...
      {exportData && (
        <Dialog open={!!exportData} onOpenChange={() => setExportData("")}>
          <DialogContent className="max-w-2xl">
            <DialogHeader>
              <DialogTitle>Export {selectedMix ? `"${selectedMix.name}"` : "All Mixes"}</DialogTitle>
            </DialogHeader>
            <div className="space-y-4">
              <div>
                <Label>JSON Data</Label>
                <Textarea value={exportData} readOnly rows={15} className="font-mono text-sm" />
              </div>
