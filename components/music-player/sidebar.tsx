...
export function Sidebar({ songs, selectedSongs, onSongSelect, onAnalyzeSongs }: SidebarProps) {
  console.log(
    "[v0] Sidebar - songs:",
    songs.map((s) => s.title),
  )
  ...
  return (
    <div className="w-80 bg-sidebar border-r border-sidebar-border p-4 flex flex-col">
      <div className="mb-6">
        <h2 className="text-lg font-semibold text-sidebar-foreground mb-4">Music Library</h2>
        <p className="text-xs text-muted-foreground mb-4">
          Select songs below to analyze their sections and create custom mixes
        </p>

        {selectedSongs.length > 0 && (
          <Button onClick={onAnalyzeSongs} className="w-full mb-4" variant="default">
          ...
          return (
            <Card
              ...
              className={`p-3 cursor-pointer transition-colors hover:bg-sidebar-accent ${
                isSelected ? "bg-sidebar-accent border-primary" : "bg-sidebar-primary"
              }`}
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <h3 className="font-medium text-sidebar-primary-foreground truncate">{song.title}</h3>
                  </div>

                  <p className="text-sm text-muted-foreground mb-2">{song.artist}</p>
                  <p className="text-xs text-muted-foreground mb-2">Duration: {formatDuration(song.duration)}</p>

                  {song.sections && song.sections.length > 0 && (
                    <div className="flex flex-wrap gap-1">
                      {song.sections.map((section) => (
                        <Badge key={section.id} variant="outline" className="text-xs text-foreground bg-background">
                          {section.type}
                        </Badge>
                      ))}
                    </div>
